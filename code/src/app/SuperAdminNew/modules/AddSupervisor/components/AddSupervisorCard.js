...
import AddSupervisorTable from "./supervisor-details-table/AddSupervisorTable";
import {fireAlert} from "../../../../../utils/SuperAdmin/SweetAlert";
import {FireAlertMessage} from "../../../../../utils/SuperAdmin/enums/firAlert.enums";
import {AllCompanyOption, AllSupervisorOption} from "../../../../../utils/SuperAdmin/enums/CompanyOption";
import {AssignLocationDialog} from "./supervisor-details-edit-dialog/AssignLocationDialog";
...
export function AddSupervisorCard() {
    ...
    const [loadingAssignModalData, setLoadingAssignModalData] = useState(false)
    ...
    useEffect(() => {
        setCurrentItems(AllSupervisorOption);
    }, [AllSupervisorOption]);
    ...
    const handleServiceAccessStatus = row => {
        ...
        //   status: !row?.status
        // };
        // dispatch(action.updateSupervisorStatuss(newData, row?.id)).then(() => {
        //   if(storedActiveOrganization){
        //     const org_id = JSON.parse(storedActiveOrganization)
        //     dispatch(action.getSupervisorByOrganizations(org_id?.id));;
        //   }
        // });
    };

    const handleDeleteSupervisor = row => {
        fireAlert(
            row,
    ...
    const handleAssignLocationModalClose = () => {
        setEditModalData([]);
    ...
    const editModalDataTable = row => {
        ...
        //   dispatch(action.getSupervisorByIds(row?.id))
        //       .then(() => {
    ...
    return (
        <>
                    ...
                    <Row>
                        <Col xl={12} style={{padding: "10px 20px", minWidth: "300px"}}>
                            ...
                            <AddSupervisorTable
                                ...
                                deleteModalDataTable={handleDeleteSupervisor}
                            />
                            {/*      </div>*/}
                            {/*    </>*/}
                            {/*)}*/}
            ...
            {showModal && <> <AddSupervisorEditDialog show={showModal} onHide={handleClose}
                                                      selectedCompany={selectedCompany}
                                                      editModalData={editModalData}
                                                      loadingModalData={loadingModalData}/></>}

            {showAssignLocationModal && <> <AssignLocationDialog show={showAssignLocationModal}
                                                                 editModalData={editModalData}
                                                                 onHide={handleAssignLocationModalClose}
                                                                 loadingModalData={loadingAssignModalData}/> </>}
        </>
    );
}
